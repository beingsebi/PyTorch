...
b = torch.randn(3,2)
```
`torch.mm(a,b)` $\Rightarrow$ runtime error
...
c) `torch.matmul()` (and `@`) can also perform *dot product* and *matrix-vector product*
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg0MDIyNzY1NywtOTU4NjYyNjY0LC0xNz
